...
@section('title', 'Undertime Letter Generation')
@section('content')

<div class="mb-6">
    <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Generate Undertime Letter</h1>
    <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Generate a formal notice letter for employee undertime records.</p>
</div>

<div class="max-w-4xl"
     x-data="undertimeLetterForm({{ $allUsers->toJson() }})">
    ...
    <form method="POST" action="{{ route('admin.undertime-tardy.undertime-letter.generate') }}">
        @csrf
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm mb-5">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="position" x-model="selectedPosition" readonly
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="office" x-model="selectedOffice" readonly
                           placeholder="Auto-filled from employee"
                           class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-100 dark:bg-slate-700/60
                                  text-slate-600 dark:text-slate-300 text-sm focus:outline-none cursor-not-allowed"/>
                </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm mb-5">
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center gap-2">
                ...
                <h2 class="text-sm font-semibold text-slate-700 dark:text-slate-300">Letter Details</h2>
            </div>
            <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-5">
                    ...
                    <div class="relative">
                        <select name="month" required
                                class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700
                                       text-slate-800 dark:text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition appearance-none">
                            @for($m=1; $m<=12; $m++)
                                <option value="{{ date('F', mktime(0, 0, 0, $m, 1)) }}" {{ date('n') == $m ? 'selected' : '' }}>
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">
                        No. of Undertime Occurrences <span class="text-red-400">*</span>
                    </label>
                    <input type="text" name="undertime_count" required
                    ...
                    <p class="text-xs text-slate-400 mt-1">Appears as: "You have incurred ___ times undertime during the month of [Month and Year]."</p>
                </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm mb-6">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="certifier_name" value="AIDA B. LOVERES"
                           class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700
                                  text-slate-800 dark:text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition"/>
                </div>
                <div>
                    ...
                    <input type="text" name="certifier_position" value="PG Department Head/PHRM Officer"
                           class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700
                                  text-slate-800 dark:text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition"/>
                </div>
            </div>
        ...
        <div x-show="selectedId" x-transition style="display:none"
             class="mb-5 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl text-sm text-blue-700 dark:text-blue-300">
            <p class="font-semibold mb-0.5">Letter will be addressed to:</p>
            <p>Dear <span x-text="'[Prefix] ' + query"></span>,</p>
            <p class="text-xs mt-1 text-blue-500 dark:text-blue-400">Position: <span x-text="selectedPosition || '—'"></span> &nbsp;|&nbsp; Office: <span x-text="selectedOffice || '—'"></span></p>
        </div>

        {{-- ── Submit ── --}}
        <div class="flex justify-end gap-3">
            <p class="text-xs text-slate-400 dark:text-slate-500 self-center">
                ✦ Letter will be saved to <strong>Generated Letters</strong> — then you can print from there.
            </p>
            <button type="submit"
...
<script>
function undertimeLetterForm(employees) {
    return {
        employees,
        query: '',
        filtered: [],
